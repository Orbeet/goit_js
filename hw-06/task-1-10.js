...
//   return users
//     .filter(users => users.friends.some(friends => friends === friendName))
...
const getSortedUniqueSkills = users => {
  let resArray = [];
  const newArray = users.map(el => (resArray = [...resArray, ...el.skills]));
  const skillsArr = resArray.filter((item, idx) => {
    return idx === resArray.indexOf(item);
  });
